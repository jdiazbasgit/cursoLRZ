...
export default class Paises extends Component {
    ...
    botonGeografico = () => {
        this.setState({ h1geograficos: this.props.pais?.area })
    }
    ...
    render() {
        return (
            <div>
                ...
                Continente:
                <select id="continente" onChange={this.recuperaContinente}>
                    {this.continentes.map(continente => {
                        return <option value={continente.valor}>{continente.texto}</option>
                    })}
                </select>

                <select id="pais" onChange={this.cargarPais}>
                ...
                <button onClick={this.botonGeografico}>{this.state.botonGeograficos}</button>
                <h1>{this.state.h1geograficos}</h1>
            </div>
        )
    }
}